...
class Video():
    ...
    def find_best_audio(self, max_abr=None):
        ...
        for each in self.formats:
            if (
                ...
                (max_abr is None or each['abr'] <= max_abr)
            ):
    ...
    def find_best_video(self, max_height=None):
        ...
        for each in self.formats:
            if (
                ...
                (max_height is None or
                    (each['height'] != None and each['height'] <= max_height))
            ):
                format_filesize.append(
                    each['format_id'],
                    each['height'],
                    each['filesize']
                    )
    ...
    def find_best_av(self, max_height=None):
        ...
        for each in self.formats:
            if (
                ...
                (max_height is None or each['height'] <= max_height)
            ):
    ...
    def play_with_mpv(self, timeout=10):
        ...
        while count >= 0:
            ...
            if count == 0:
                print(
                    'Youtube Download:' +
                    'File did not reach 4MB, attempting to play'
                    )
                break
    ...
    def download_and_play_av(self):
        self.q = Queue()
        self.download_process = start_thread(
            self.download, self.find_best_av(), self.q
            )
    ...
    def download_and_mux_best_av(self, max_height=1080):
        try:
            self.download(
                self.find_best_video(max_height) + '+' +
                self.find_best_audio()
                )
            self.play_with_mpv()
        except:
